...
IMDbCategory = Literal["most_popular", "top_rated", "in_theaters"]
...
def _abs(url: str) -> str:
    return url if url.startswith("http") else f"https://www.imdb.com{url}"
...
def fetch_imdb_list(category: IMDbCategory = "most_popular", limit: int = 20) -> List[Dict[str, Any]]:
    resp = session.get(IMDB_URLS[category], timeout=20)
    ...
    if category in ("most_popular", "top_rated"):
        ...
        for row in rows:
            try:
                ...
                movies.append(Movie(title, year, rank, rating, url_full, poster))
                if len(movies) >= limit:
                    break
            except Exception:
                continue
        ...
        for idx, c in enumerate(cards, start=1):
            ...
            if not title:
                continue
            movies.append(Movie(title, None, idx, None, url_full, poster))
            if len(movies) >= limit:
                break
